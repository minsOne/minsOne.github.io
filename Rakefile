...
task :post do
  ...
  filename = File.join(CONFIG['posts'], (ENV['date'] ? Time.parse(ENV['date']) : Time.now).strftime('%Y'), (ENV['date'] ? Time.parse(ENV['date']) : Time.now).strftime('%m'), "#{date}-#{slug}.#{CONFIG['post_ext']}")
