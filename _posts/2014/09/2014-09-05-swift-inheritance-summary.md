...
오버라이드 속성의 부분으로서 setter을 준다면, 오버라이드를 위한 getter을 줄 수 있음. 오버라이딩 getter안에 상속받은 속성의 값을 수정하길 원치 않는다면 상속받은 값에 getter로부터 `super.someProperty`를 반환하도록 넘겨줄 수 있으며, someProperty는 오버라이딩하는 속성 이름임.
...
상속받은 상수 저장 속성이나 읽기 전용 계산 속성에는 속성 감시자를 추가할 수 없음. 이들 속성의 값은 설정되지 못하며, 오버라이드의 한 부분으로 `willSet` 또는 `didSet` 구현은 적합하지 않음.
