...
[이전 글](../ios-swiftpm-rxblocking-rxtest-on-unit-test)에서는 모듈 중복 적재되는 문제를 코드 복사로 풀었습니다. 하지만 Xcode 13.2.1에서는 모듈 중복 적재되는 문제를 코드 복사가 아닌 SwiftPM으로 잘 풀 수 있게 된 것 같습니다. 

Xcode 13.0을 받아서 아래에 기술할 방법을 사용하였을 때도 되는 것 같지만, 애매하여 Xcode 13.2.1 이상 버전에서 작업하는 것을 추천드립니다.
...
유닛 테스트 모듈이 RxSwift를 의존성 가진다면 다음과 같은 구조가 됩니다.

<p style="text-align:left;"><img src="{{ site.production_url }}/image/2022/01/20220103_01.png"/></p>

유닛 테스트 모듈은 RxSwift, RxCocoa, RxRelay를 SwiftPM을 이용하여 직접 의존성을 가지도록 하였습니다.

아래 그림은 유닛 테스트 타겟의 Build Phase에서 RxSwift, RxCocoa, RxRelay, RxBlocking, RxTest, Nimble, RxNimble, Quick을 SwiftPM으로 직접 추가한 상태입니다.

<p style="text-align:left;"><img src="{{ site.production_url }}/image/2022/01/20220103_02.png"/></p>

그리고 테스트 소스에서 ThirdPartyLibraryManager와 RxSwift, RxCocoa, RxRelay, RxBlocking, RxTest, Nimble, RxNimble, Quick를 import 하도록 합니다.

<p style="text-align:left;"><img src="{{ site.production_url }}/image/2022/01/20220103_03.png"/></p>

테스트를 실행하였을 때, RxSwift 모듈이 중복해서 적재되었다는 경고가 전혀 발생하지 않았습니다.

<p style="text-align:left;"><img src="{{ site.production_url }}/image/2022/01/20220103_04.png"/></p>

## 그 이유는?

해당 문제가 이슈화된지는 오래되었습니다. 버그 리포트 [https://bugs.swift.org/browse/SR-12303](https://bugs.swift.org/browse/SR-12303)도 되었고 RxSwift 저장소의 오래된 [이슈](https://github.com/ReactiveX/RxSwift/issues/2127)였기도 했습니다. 트위터에서도 어떻게 해결해야할지 [이야기](https://twitter.com/freak4pc/status/1233465169189228544?s=20)도 있었고요.

[Xcode 12.5 Release Note]((https://developer.apple.com/documentation/xcode-release-notes/xcode-12_5-release-notes#Swift-Packages))에서 Swift Package에 다음 항목이 있습니다.
...
라이브러리가 중복되는 문제를 `Dynamic Framework로 해결`했음을 이야기합니다. 즉, 보통의 경우 앱 타겟으로 빌드시는 중복이 일어날 일이 없기 때문에 상관없지만, 유닛 테스트 타겟으로 빌드할때는 RxSwift와 같이 라이브러리가 중복될 수 있습니다.

그럼 어떻게 해결했는지 테스트 빌드한 결과물을 보고 확인해봅시다.
...
이 파일의 패키지 내용을 확인하면 실행 바이너리와 프레임워크를 확인할 수 있습니다.

<p style="text-align:left;"><img src="{{ site.production_url }}/image/2022/01/20220103_07.png"/></p>

프레임워크 폴더에 `RxSwift.framework`, `RxRelay.framework`, `RxCocoa_38E61CAF42DDE0B6_PackageProduct.framework` 가 있습니다. SwiftPM으로 서드파티 라이브러리를 추가하면 보통은 Static Library로 추가되기 때문에 Framework가 만들어지지 않지만, 프레임워크가 만들어졌습니다.

SampleAppTests 바이너리가 RxSwift, RxRelay, RxCocoa 프레임워크를 링킹하고 있는지 확인해봅시다.
...
ThirdPartyLibraryManager 프레임워크 내에 Frameworks 폴더가 생성되었고, RxSwift, RxRelay, RxCocoa 프레임워크가 있음을 확인할 수 있습니다.
